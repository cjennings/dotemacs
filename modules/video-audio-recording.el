...
;; The toggles live only under C-; r.  They had a fast chord (F9 video, S-F9
;; audio, claimed in every EAT map too) until 2026-09-22: with Fn Lock on the
;; Framework 13 the F-key row sends plain function keys, and a bare F9 press
;; started a screen recording.  Both chords went together, because a lone S-F9
;; on a live F-key row is one fat-finger from an unwanted audio recording.
;; Both commands still take a prefix argument, so C-u C-; r v prompts for the
;; recording location.
